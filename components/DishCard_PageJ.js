...
function Description({name}){
  return (
    <div className="w-[150px] h-[60px] ml-[13px] flex flex-col ">
      <div className="w-full h-[15px] mb-[10px] text-white text-xl">
        <p> {name}</p>
      </div>
      <div className="ml-[-3px] w-full h-[20px] flex">
        ...
        <p className=' mt-[3px] text-sm text-white'> {Number.parseFloat(Math.random()*(4.50-3.30)+(3.30)).toFixed(2)} </p>
      </div>
  ...
  return (
    <div className="w-[80px] h-[60px] ml-[12px] mr-[12px] ">
      <div className="w-full h-[30px] mt-[-2px] flex items-end flex-row-reverse text-white ">
        <p >${prices[Math.floor(Math.random() * (21 - 0 + 1) + 0)]}</p>
      </div>
